...
class HabrSpider(scrapy.Spider):
    ...
    def parse_vacancy(self, response):
        ...
        out = {
            "url": response.url,
            "Title": sub_response.xpath(
                './/h1[@class="page-title__title"]/text()'
            ).get(),
            "Company.Name": sub_response.xpath(
                'normalize-space(.//div[@class="company-name"])'
            ).get(),
            "Company.Link": sub_response.xpath(
                './/div[@class="company-name"]//a/@href'
            ).get(),
            "Description.HTML": sub_response.xpath(
                './/div[@class="job_show_description__body"]'
            ).get(),
        ...
        for section in sub_response.xpath('.//div[@class="content-section"]'):
            ...
            if t == "Зарплата":
                ...
                out[t] = [
                    item.xpath("normalize-space(.)").get()

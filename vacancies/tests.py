...
def test_create_blog_view(new_user_verified):
    c = Client()
    response = c.post(
        reverse("login"), {"username": test_username, "password": test_password}
    )
...
def test_register(
    username, first_name, last_name, email, password1, password2, will_registered
):
    c = Client()
    response = c.post(
        reverse("register"),
